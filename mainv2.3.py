...
MIN_SAMPLES_PER_CLASS = 50 # 각 클라이언트가 클래스별로 보장받는 최소 샘플 수
RANDOM_SEED = 42           # 재현 가능한 실험을 위한 랜덤 시드

# --- 버전 2: 기여도 측정 파라미터 ---
NUM_COMMITTEE_MEMBERS = 3  # 학습 노드의 성능을 평가할 위원회(Committee)의 수
EWMA_BETA = 0.5            # 정확도를 보정할 지수가중이동평균(EWMA)의 반영률 조정 파라미터
CONTRIBUTION_SCALING_FACTOR = 1.3 # 기여도 점수 차이를 증폭시키는 배수 (1.0 이상 권장)
...
def save_results(output_dir, partition_report_df, args, poisoned_classes: Set[int]):
    ...
    with open(results_filepath, "w", encoding="utf-8") as f:
        ...
        # [수정] 전체 정확도와 오염 클래스 정확도를 하나의 테이블로 통합하여 저장
        f.write("="*80 + "\n 라운드별 글로벌 모델 정확도\n" + "="*80 + "\n")
...
# ==============================================================================
def main():
    parser = argparse.ArgumentParser(description="버전 2.5: 기여도 및 악의적 노드 시뮬레이션")
    ...
    print("=" * 80, "\n버전 2.5: 기여도 기반 연합학습 시뮬레이션을 시작합니다.\n", "=" * 80)
